...
async function checkAuth(){
  try{
    ...
    if(r.ok) location.href='index.html'; else gatekeeper.classList.add('hide');
  }catch{gatekeeper.classList.add('hide')}
}
...
async function login(e){
  ...
  try{
    const r=await fetch(`${ENDPOINT}/account/sessions/email`,{method:'POST',headers:HEADERS,credentials:'include',body:JSON.stringify({email,password:pwd})});
    if(!r.ok) throw new Error();
    setFeedback('success','✓ Login successful!'); setTimeout(()=>location.href='index.html',800);
  }catch{setFeedback('error','Incorrect email or password.');btn.disabled=false}
}
...
async function signup(e){
  ...
  try{
    const r=await fetch(`${ENDPOINT}/account`,{method:'POST',headers:HEADERS,body:JSON.stringify({userId:'unique()',email,password:pwd,name})});
...
async function forgot(e){
  ...
  try{
    await fetch(`${ENDPOINT}/account/recovery`,{method:'POST',headers:HEADERS,body:JSON.stringify({email,url:'https://khabardarjeeling.space/reset-password.html'})});
